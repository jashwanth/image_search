...
//				cout << ".";
	...
	FileStorage fs("training_descriptors_surf.yml", FileStorage::WRITE);
	fs << "training_descriptors_surf" << training_descriptors;
	fs.release();
	BOWKMeansTrainer bowtrainer(no_of_clusters); //num clusters
	bowtrainer.add(training_descriptors);
	cout << "cluster BOW features" << endl;
	Mat vocabulary = bowtrainer.cluster();
	FileStorage fs1("vocabulary_surf.yml", FileStorage::WRITE);
	fs1 << "vocabulary_surf" << vocabulary;
	fs1.release();
	return 0;
}
