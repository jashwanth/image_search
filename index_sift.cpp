...
int main(int argc,char** argv)
{
  ...
  Mat bowDescriptor;
  FileStorage fs("vocabulary_sift.yml", FileStorage::READ);
  Mat vocal,img;
  fs["vocabulary_sift"] >> vocal;
  fs.release();
...
//  vector<set<string> > inv_index(vocal.rows);
  ...
  FileStorage fs1("inverted_index_sift.yml", FileStorage::WRITE);
  fs1<<"inverted_index_sift"<<inv_index;
